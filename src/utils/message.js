...
function msgFn(global) {
   ...
   * 命名空间 用于css和事件
   */
  const NAMESPACE = (global && global.MSG_GLOBALS && global.MSG_GLOBALS.NAMESPACE) || PLUGIN_NAME
  ...
  function Msg(opts) {
    ...
    $elem.innerHTML = '<div class="qmsg-content"><div class="' + contentClassName + '"><i class="qmsg-icon">' + $svg + '</i>' + $span.outerHTML + $closeIcon + '</div></div>'
...
## 全局配置
在引入message.js之前可以通过全局变量 MSG_GLOBALS.DEFAULTS 来进行配置
```javascript
window.MSG_GLOBALS = {
    DEFAULTS:{
        showClose:true,
